...
| File | Responsibility |
|---|---|
| `js/data.js` | `GraphStore` class — parses JSON, validates schema, builds adjacency index, auto-detects node types / root types / edge rels, infers depth for reversed-edge types, assigns colours, manages expand/collapse state, computes visible subset, search, `countForType()` / `countForRel()`, discovers numeric/categorical attrs, provides attr-driven colour/size/opacity mapping with selectable scale (linear/log/percentile), attribute rollups (sum/max aggregation of descendant numeric values onto ancestors, DAG-diamond-safe and cycle-safe), multi-parent type detection, connected-component ids and grouping state (`groupingEnabled` / `groupBy` / `groupKeyFor` / `groupLabel` / `groupColor` / `visibleGroups` / `groupCenters`) |
| `js/layouts.js` | Pure discrete layout functions (circle, grid, concentric, radial tree, AVSDF circular) plus `groupedDiscreteLayout()` (per-cluster packing wrapper) and the layout registry (`ALL_LAYOUTS`, `LAYOUT_LABELS`). No D3 dependency — unit-testable in Node |
| `js/graph.js` | `GraphRenderer` class — D3 force simulation, SVG rendering, zoom/pan, drag, zoom-dependent label visibility, adaptive edge rendering (including attr-weighted edges), multi-parent visual marker (yellow dashed stroke), cluster grouping hulls/regions (`.hulls` group behind edges, generalised `forceCluster(keyFn)`), pulls all visual config from `GraphStore`, switches between force-directed and discrete layouts via `setLayout()` |
| `js/ui.js` | Pure functions for UI components — stats bar, type filters (with counts and editable colour pickers), edge legend (per-rel colour/dash swatches with counts and editable colour pickers), attr selectors (colour-by / size-by dropdowns), rollup controls (enable checkbox + sum/max aggregator selector), colour scale selector (linear/log/percentile), layout selector dropdown, grouping controls (enable checkbox + group-by dropdown), colour legend (gradient bar or categorical swatches, all editable), search wiring (combobox pattern with keyboard navigation), tooltip (with multi-parent indicator and rollup value), detail modal content (with rollup value), collapsible sidebar sections with ARIA |
| `js/main.js` | Entry point — file loading (drag-and-drop + picker), wires store → renderer → UI, owns selection state and highlight logic, shows/hides the detail modal over the graph on node selection, hides force controls when a discrete layout is active, help modal (open via button or `?` shortcut, focus trapping, close on Escape/backdrop), announces grouping enable/disable |
...
- **State lives in `GraphStore`**: the renderer is stateless — call `update()` with new visible data after any store mutation. Colours and sizes are accessed via `store.nodeColor()`, `store.colorForType()`, `store.colorForRel()`, `store.nodeRadius()` etc. Selection state (`selectedNodeId`) lives on `store.selectedNodeId`, not in `main.js` or the renderer.
- **Renderer is reused across file loads**: `main.js` creates the `GraphRenderer` once and calls `renderer.reset()` on subsequent loads. `reset()` clears rendered selections, arrow markers, simulation nodes, layout state, and zoom transform, but keeps the SVG element, zoom behaviour, drag handler, ResizeObserver, and event wiring alive. Never remove the `<svg>` element manually.
- **Colour overrides persist across loads**: `setTypeColor()` / `setRelColor()` write to `_typeColorOverrides` / `_relColorOverrides` Maps that survive `load()`. The auto-assigned defaults in `typeColors` / `relColors` are rebuilt per graph, but `colorForType()` / `colorForRel()` consult the override maps first. Attr-driven overrides (`_catColorOverrides`, `_heatRampOverride`) are still reset per attr change and per load.
- **Multi-parent markers are precomputed**: `hasMultipleParentTypes()` is a `Set.has()` lookup — the Set is built once during `load()` from `parentsOf`. Never walk parents per frame.
- **Renderer accesses store through public API only**: the renderer never touches `store.parentsOf`, `store.childrenOf`, `store.edgesFrom`, `store.edgesTo`, or `store.nodeMap` directly. Use `store.parentIds()`, `store.childrenIds()`, `store.edgesForNode()`, and the other public getters. `radialTreeLayout` takes a `(id) => string[]` callback, not a raw Map.
- **Static button bindings are hoisted**: `btnExpandAll`, `btnCollapseAll`, `btnPause`, `btnSidebarToggle`, and `toggleLabels` are bound once at module level in `main.js`, not re-assigned on every `loadGraph()`. The callbacks guard against `renderer === null`.
- **Edge weights are precomputed per update**: `GraphRenderer.update()` builds `this._edgeWeights` (Map from join key to weight) so `store.edgeWeight()` is called once per edge, not four times. `highlight()` reuses the same Map. Cleared in `reset()`.
- **Aria labels derive from `LAYOUT_LABELS`**: `GraphRenderer._ariaLabel()` uses `LAYOUT_LABELS[this._layout]` from `layouts.js` as the single source of truth. Never hardcode layout names in `graph.js`.
- **Label radii are cached at creation**: `update()` stores `d._labelRadius` on each label datum so `_updateLabelVisibility()` doesn't recompute `store.nodeRadius()` (Map.get + sqrt) on every zoom event.
- **Attr state reset is centralized**: `GraphStore._resetAttrState()` resets all attr-mapping and rollup state. Called during `load()`. Never clear attr fields individually.
- **Typecheck is enforced**: `npm run typecheck` runs `tsc -p jsconfig.json` and must pass with zero errors. The `jsconfig.json` has `checkJs: true` and `strict: false`. JSDoc types are required on all public APIs and non-trivial private fields.
...
- **Attribute rollups**: when a numeric colour-by or size-by attr is active, users can enable "Roll up descendant values" to aggregate the attr across each node's sub-tree (self + all descendants) so the mapping works above the leaf layer. The aggregation function is Sum (default) or Max. Rollup values are computed in `_rebuildRollup()` as the aggregate over each node's unique descendant set — shared descendants in DAG diamonds are counted once, and cycles are handled by BFS reachability (each node visits its descendants at most once per traversal). Descendant sets are topology-only and cached in `_descendantSets` via `_ensureDescendantSets()` — they are computed once per `load()` and reused across all attr/fn changes. Rollup values are additionally cached per attr key (and tagged by fn) in `_rollupCacheByAttr`, so switching back to a previously-used attr restores its values without recomputing descendant sets. When rollup is active, `getColorLegend()` reports the rolled-up range, `edgeWeight()` uses the target node's rolled-up value, and `nodeOpacity()`/`nodeColor()`/`nodeRadius()` reflect rolled-up values so ancestors without the raw attr still get a real colour/size. `rollupValue(node)` returns the aggregate (or undefined when rollup is off / the sub-tree has no value).
...
- **Grouping is a store-owned overlay**: grouping state (`store.groupingEnabled` / `store.groupBy`) and connected-component ids (`store._componentId`, computed once at `load()` via union-find over the full edge list) live in `GraphStore`. The renderer draws convex-hull hulls behind edges in a `.hulls` group for the force layout, and reuses `groupedDiscreteLayout()` from `layouts.js` to pack clusters into labelled regions for discrete layouts. Single-member clusters get no hull or label. Hull label visibility is a zoom-driven `display` toggle (`_updateHullLabelVisibility()`), so labels reappear when zooming back in without a hull recompute. Hull recompute is deferred to simulation settle (alpha < 0.05) above 1k visible nodes. Grouping resets to off on `load()` (alongside `_resetAttrState()`).
...
215 tests total — 166 covering the data layer (validation, loading, type/rel detection, expand/collapse, getVisible, search, reveal, nodeRadius, clusterCenters, edgesForNode, childrenIds, attribute discovery, colour-by-attr, size-by-attr, node opacity, edge weight, colour overrides, legend data, multi-root support, colour scale modes, multi-parent type detection, edge count per rel, attribute rollups: defaults/state, sum aggregation, max aggregation, DAG-diamond dedup, cycles, colour/size/edge-weight integration, load-reset, large-graph performance, grouping: component ids, group keys/labels/colours, visibleGroups, groupCenters, state reset) and 49 covering the discrete layout algorithms (circle, grid, concentric, radial tree, AVSDF circular: placement correctness, velocity reset, metric ordering, multi-root and DAG handling, zero-crossing tree layouts, local-adjusting crossing reduction; groupedDiscreteLayout: per-cluster placement, non-overlap, velocity reset, edge cases, determinism). Four graph sizes (10/100/1k/10k nodes+edges each) verify correctness and performance.
...
- `layouts.edge-cases.test.mjs` — layout persists across expand/collapse, type filter toggle, search & select, colour-by attr change; pause/resume with discrete layout; force sim restores after switching back; drag works in discrete layouts; rollup controls appear when numeric colour-by attr selected; rollup changes node colours when enabled; grouping: hull per visible type, hull count updates after filter/expand, selection survives grouping toggle, discrete regions, component grouping, singleton clusters hidden, zoom-dependent hull labels
- `layouts.large-fixture.test.mjs` — all layouts render 9,609 nodes / 13,417 edges with no browser errors; grouping renders hulls on the large fixture
- `layouts.a11y.test.mjs` — see below

20 accessibility tests in `layouts.a11y.test.mjs`:
...
- **Grouping**: a collapsible sidebar section below Layout with an "Group nodes into clusters" checkbox (`#grouping-enabled`) and a "Group by" dropdown (`#select-group-by`: Node type / Connected component, disabled while grouping is off). When enabled, visible clusters with at least two members are enclosed in translucent labelled hulls (force layout) or labelled regions (discrete layouts); the cluster force strengthens automatically so hulls stay compact. Toggling grouping or switching the group-by key calls `store.setGroupingEnabled()` / `store.setGroupBy()` and re-renders without losing the selection.
...
- Screen reader live region (`#sr-announcements`, `aria-live="polite"`) announces graph load, node selection, layout changes, grouping enable/disable, and help dialog open/close
